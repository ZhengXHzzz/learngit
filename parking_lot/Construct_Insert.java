...
public class Construct_Insert {
    ...
    public Construct_Insert(String table)
    {
        this.table = table;
    }
    ...
    public String toString()
    {
        ...
        if(values.size() > 0)
        {
            ...
            for(int i=0; i<values.size(); i++)
            {
                ...
                if(i > 0) vvv += ",";
                vvv += Construct_SQL.value( str );
            }
            vvv += ")";
        }

        String sql = " INSERT INTO " + Construct_SQL.name(table)
                + ccc
                + " VALUES "
                + vvv;
